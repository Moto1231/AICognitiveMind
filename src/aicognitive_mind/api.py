...
from aicognitive_mind.core import CognitiveCore, FoundationNotInitializedError, MindNotInitializedError
...
from aicognitive_mind.engines import EchoReasoningEngine, OllamaReasoningEngine, OpenAIReasoningEngine, ReasoningEngine
...
def authorize_admin(authorization: str | None) -> None:
    ...
    if not expected:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Administrative access is not configured")
    scheme, separator, supplied = (authorization or "").partition(" ")
    valid = separator == " " and scheme.lower() == "bearer" and bool(supplied) and secrets.compare_digest(supplied, expected)
    if not valid:
        raise HTTPException(
...
async def ensure_foundation(foundation: MongoFoundationStore, key: str, content: str) -> None:
    active = await foundation.load_active(key)
    if active is None:
        await foundation.seed(key, content)
    elif active.content != content:
        await foundation.revise(key, content, changed_by="bootstrap_upgrade")
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    await ensure_foundation(foundation, CONSCIOUS_WORKSPACE_FOUNDATION_KEY, CONSCIOUS_WORKSPACE_FOUNDATION_SEED)
    await ensure_foundation(foundation, MEMORY_STEWARD_SYNTHESIS_FOUNDATION_KEY, MEMORY_STEWARD_SYNTHESIS_FOUNDATION_SEED)
    await ensure_foundation(foundation, CONSCIOUS_EXPRESSION_FOUNDATION_KEY, CONSCIOUS_EXPRESSION_FOUNDATION_SEED)
    ...
    app.state.core = CognitiveCore(
        ...
        engine=engine,
        working_memory=MongoWorkingMemoryStore(runtime.database),
...
@app.post("/v1/mind/initialize", response_model=CognitiveMind, status_code=status.HTTP_201_CREATED)
async def initialize_mind(body: InitializeMindRequest, request: Request) -> CognitiveMind:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="This instance already contains its mind") from exc
...
async def load_mind(request: Request) -> CognitiveMind:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The mind has not been initialized") from exc
...
async def interact(body: InteractionRequest, request: Request) -> InteractionResult:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="A required governed foundation is unavailable") from exc


@app.get("/v1/mind/working-memory", response_model=WorkingMemoryState)
async def read_working_memory(request: Request) -> WorkingMemoryState:
    try:
        return await get_core(request).read_working_memory()
    except MindNotInitializedError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The mind has not been initialized") from exc


@app.post("/v1/mind/checkpoint", response_model=WorkingMemoryState)
async def checkpoint(request: Request) -> WorkingMemoryState:
    try:
        return await get_core(request).checkpoint()
    except MindNotInitializedError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="The mind has not been initialized") from exc
...
@app.get("/v1/admin/foundation/{key}", response_model=list[FoundationalMemory])
async def read_foundation_history(
    key: str, request: Request, authorization: Annotated[str | None, Header()] = None
) -> list[FoundationalMemory]:
    authorize_admin(authorization)
    return await get_foundation(request).read_history(key)


@app.put("/v1/admin/foundation/{key}", response_model=FoundationalMemory)
async def revise_foundation(
    ...
    authorize_admin(authorization)
    return await get_foundation(request).revise(key=key, content=body.content, changed_by="administrator")
