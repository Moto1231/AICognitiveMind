...
async def ensure_foundation(
    foundation: MongoFoundationStore,
    key: str,
    content: str,
) -> None:
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    if provider == "ollama":
        ...
        if not settings.openai_api_key:
            raise RuntimeError(
                "OPENAI_API_KEY is required when REASONING_PROVIDER=openai"
            )
...
async def initialize_mind(body: InitializeMindRequest, request: Request) -> CognitiveMind:
    try:
        return await get_core(request).initialize(
            body.self_name,
            body.foundational_values,
        )
